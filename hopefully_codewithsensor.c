...
int main (void)
{
	...
	int v = 30; //rough operating range of 1 to 5V, set distance set in middle (3.0V)
	int toggle = 0;
	int deadzone = 200; //0.2V deadzone
  long count;
	float distance;
  ...
  sei();			/* Enable global interrupt */
    ...
    usart_pstr("distance:");
    PrintNumber(integer_part, 16, 3);
    usart_pstr(".");
    PrintNumber(fractional_part, 16, 3);
		_delay_ms(200);
	...
	//PRIORITIZE TURNING LEFT AND RIGHT OVER FORWARD AND BACK
		
		if((toggle == 1) &&((PINC&0b0001000) == 0)||(toggle == 0 && ((v1 < (v*100 - deadzone)) && (v2 > (v*100 + deadzone)))))
		{//Left
			//PORTD = 0b01010000;
			PORTD |= ();
			PORTD &= ~(0b00101000);	
		...
		else if((toggle == 1) &&((PINC&0b0100000) == 0)||(toggle == 0 && ((v1 > (v*100 + deadzone)) && (v2 > (v*100 + deadzone)))))
		{//Forward
			//PORTD = 0b01001000;
			PORTD |= (0b01001000);
			PORTD &= ~(0b00110000);

...
void main (void)
{
	...
	while(1) {
		...
		else if (joy_x >= 50 && (joy_y * 2 < joy_x && -joy_y * 2 < joy_x)) { // right
			LCDprint("          ^     ", 2, 1);
			new_off_period = MIN_OFF_TIME + 2 * TIME_STEP;
		}
		else if (joy_y >= 50 && (joy_x * 2 < joy_y && -joy_x * 2 < joy_y)) { // forward
			LCDprint("^               ", 2, 1);
			new_off_period = MIN_OFF_TIME + 3 * TIME_STEP;
		}
		else if (joy_x <= -50 && (joy_y * 2 < -joy_x && -joy_y * 2 < -joy_x)) { // left
			LCDprint("      ^         ", 2, 1);\
			new_off_period = MIN_OFF_TIME + 4 * TIME_STEP;
		}
		else if (joy_y <= -50 && (joy_x * 2 < -joy_y && -joy_x * 2 < -joy_y)) { // back
			LCDprint("   ^            ", 2, 5);

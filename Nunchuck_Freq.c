...
#define  DEF_FREQ       16193L

#define  OUT0           P2_0
#define  OUT1           P2_1

#define  MIN_ON_TIME 	64 // 48  // 1.5 ms
#define  MIN_OFF_TIME	64 // 48  // 1.5 ms
#define  DEFAULT_TIME	128 // 100 // 3 ms
...
void calc_on_time(int y) 
{
	on_period = y + DEFAULT_TIME + MIN_ON_TIME;
}

void calc_off_time(int x) 
{
	off_period = x + DEFAULT_TIME + MIN_OFF_TIME;
}
...
void main (void)
{
	...
	while(1) {
		...
		if (joy_x <= 100 && joy_x >= 80) {
			LCDprint("          ^", 2, 1);
		}
		else if (joy_y <= 100 && joy_y >= 80) {
			LCDprint("^          ", 2, 1);
		}
		else if (joy_x <= -80 && joy_x >= -100) {
			LCDprint("      ^    ", 2, 1);
		}
		else if (joy_y <= -80 && joy_x >= -100) {
			LCDprint("   ^       ", 2, 1);
		}
		else {
		 	LCDprint("          ", 2, 1);
		...
		printf("Buttons(Z:%c, C:%c) Joystick(%4d, %4d) Accelerometer(%3d, %3d, %3d) Period(%d, %d) follow(%c) \x1b[0J\r\n",
			   but1?'1':'0', but2?'1':'0', joy_x, joy_y, acc_x, acc_y, acc_z, on_period, off_period, (flags&FOLLOW_MODE)?'1':'0');
		Timer4ms(100);
   }
}
